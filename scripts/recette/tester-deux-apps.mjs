...
/* Combien de fiches SONT réellement ouvertes, d'après ce qu'affiche la liste :
   ...
   Tout poste ouvert porte « Disponible » — publié sur le site ou ouvert en
   mobilité interne, échéance proche ou non. */
const badges = await p.locator('.poste-row__tags').allInnerTexts();
const ouverts = badges.filter((t) => /disponible/i.test(t)).length;
